...
// Estilos por tipo de operação (fácil de expandir)
...
export default function SinalCard({
  sinal,
  isNext = false,
  isRead = false,
  onMarkRead,
  ...
  const badgeText = operacao || '—'
  const badgeClasses = `text-2xl font-bold ${style.badge}`

  const dt =
    sinal?.parsedHorario instanceof Date
      ? sinal.parsedHorario
      : (sinal?.horario ? new Date(sinal.horario) : new Date())
